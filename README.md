...
The columns are outcome, name, input, expectations and reason, padded to the widest row printed. The name is the fixture's own, the files beside it are the suffixes it carries — the HTML fixtures pin two expectations, the other thirty-one pin one — and the reason comes last because it is the only field holding spaces. The expectations are comma-joined without one, so every row has the same field count and the second column is a name that can be fed straight back in:
